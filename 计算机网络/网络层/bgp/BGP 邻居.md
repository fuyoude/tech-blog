...
- 可选非过渡（Optional Non-transitive）：BGP 设备不一定需要识别此类属性，在 **`UPDATE`** 报文中属于可选包含。如果设备无法识别该属性，其处理机制是直接丢弃该属性，不再继续向后传递，路由继续通告给其他对等体。典型的可选非过渡属性包括 **`MED`**、**`ORIGINATOR_ID`** 和 **`CLUSTER_LIST`**。
...
通过 Open 消息告知 BGP 对等体本地支持路由刷新能力（Route-refresh capability）。在所有 BGP 路由器使能 **`Route-refresh`** 能力的情况下，**<font color="red">如果 BGP 的入口路由策略发生了变化，本地 BGP 路由器可以通过手动触发，向对等体发布 **`Route-refresh`** 消息</font>**，收到此消息的对等体会将其路由信息重新发给本地 BGP 路由器。这样，可以在不中断 BGP 连接的情况下，对 BGP 路由表进行动态刷新，并应用新的路由策略。

BGP 首次建立邻居后，会先进行一次初始路由交换，邻居建立完成后，通常只在路由发生变化时发送增量 **`UPDATE`**，并不会周期性地把全部路由重新发送一遍。如果希望本地策略修改后生效，又不重置 BGP 连接，设备可以使用 Route Refresh 扩展，请邻居重新通告路由。

入口策略处理的是邻居发送过来的完整路由信息，正常情况下，设备只保留通过入口策略后的路由。被策略拒绝的路由，或者其完整的路径属性，不会继续保存本地。例如，AR2 向 AR1 通告：
...
此时，AR1 可能只保留 **`10.1.0.0/16`** 和 **`172.16.0.0/16`**。后来管理员修改策略，允许 **`10.2.0.0/16`**，并希望将其 **`Local Preference`** 设置为 200。AR1 无法恢复此前被拒绝的 **`10.2.0.0/16`**，也无法知道该路由原本携带的完整 **`AS_PATH`**、**`COMMUNITY`**、**`NEXT_HOP`** 等属性。这就是为什么仅依靠本地 Routing Table，往往无法完整地重新应用新的入口策略。因此就需要通过 Route Refresh 让邻居重新发送这些路由。
