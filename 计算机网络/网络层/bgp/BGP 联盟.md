...
在 R5 上配置聚合命令后，聚合前的明细路由分别为 **`100.1.1.0/24 (65001) 100`** 和 **`100.1.2.0/24 (65001 65002)`**，因此聚合后的路径为：普通有序部分直接在 **`AS_PATH`** 中列出，联盟有序部分保留 **`(65001)`**，无序部分作为 AS-SET 集合，**<font color="red">联盟内的无序 AS 集合用 **`[]`** 来表示，外部无序 AS 集合用 **`{}`** 来表示，所以路径为：`(65001) [65002] {100}`</font>**。
